import 'package:flutter/material.dart';

//Colors
const COLOR_WHITE = Color(0xffFFFFFF);
const COLOR_PURPLE_CARD_BUTTON = Color(0xffC7A7F0);
const COLOR_PURPLE_TEXT = Color(0xff9469C9);
const COLOR_PURPLE_CARD = Color(0xffD4C1EC);
const COLOR_RED_CARD_BUTTON = Color(0xffA95156);
const COLOR_RED_TEXT = Color(0xffFFACB0);
const COLOR_RED_CARD = Color(0xffFF898F);
const COLOR_GREEN_CARD_BUTTON = Color(0xff2DA34B);
const COLOR_GREEN_TEXT = Color(0xffD6F6DD);
const COLOR_GREEN_CARD = Color(0xffB1EDC0);
const COLOR_GREY = Color(0xff4E4E4E);
const COLOR_WHITE_GREY = Color(0xffF4F4F4);
